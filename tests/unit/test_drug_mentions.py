...
def mock_data():
    ...
    clinical_trials_data = {
        "scientific_title": [
            "Trial involving DrugA",
            "DrugB clinical trial",
            "Effects of DrugC in patients",
        ],
...
def test_find_mentions_missing_drug_column(mock_data):
    ...
    drugs_df_missing_column = pd.DataFrame(
        {"other_column": ["DrugA", "DrugB", "DrugC"]}
    )

    # Assert that the ValueError is raised
    with pytest.raises(
        ValueError, match="The drugs dataframe must have a 'drug' column."
    ):
...
def test_find_mentions_missing_title_column(mock_data):
    ...
    # Assert that the ValueError is raised
    with pytest.raises(
        ValueError, match="Missing required 'title' column in pubmed dataframe."
    ):
