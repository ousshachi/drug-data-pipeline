...
from src.data_transform.data_processing import load_csv_files
        ...
        "PubMed": pd.DataFrame(
            {
                "id": [1, 2, 3, 4],
                "title": ["A", "B", "C", "D"],
                "date": ["2020-01-01", "2021-01-01", "2022-01-01", "2025-01-01"],
                "journal": ["A", "B", "C", "D"],
            }
        ),
        "ClinicalTrials": pd.DataFrame(
            {"id": [10, 11, 12, 13], "scientific_title": ["X", "Y", "Z", "W"]}
        ),
        "Drugs": pd.DataFrame(
            {"atccode": ["D1", "D2"], "drug_name": ["DrugA", "DrugB"]}
        ),
    }


@patch("data_extract.extract.load_csv")
